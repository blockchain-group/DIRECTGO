function [minima, xatmin, history] = sDirect_L1(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirect_L1
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_L1(Problem, bounds, opts)
...
%                 Problem.penalty     = penalty for constraint functions
...
%--------------------------------------------------------------------------
    ...
    POH = Find_po(MSS.FF(1:VAL.I) + MSS.XX(1:VAL.I), MSS.LL(:,1:VAL.I),...
        Fmin, OPTI.ep, MSS.DD(1:VAL.I));
...
getopts(opts, 'maxits', 1000,'maxevals', 100000, 'maxdeep', 1000,...
 'testflag', 0, 'globalmin', 0, 'tol', 0.01, 'showits', 1, 'ep',...
 1e-4, 'ept', 0);
...
Fmin           = MSS.FF(1) + MSS.XX(1);% initial minima
...
if isfield(Problem, 'constraint')
    ...
    for i = 1:length(con_g)
        if con_g(i) > OPTI.ept
            ret_value = ret_value + con_g(i)*Problem.penalty;
        end
    end
    % equality
    for i = 1:length(con_h)
        if abs(con_h(i)) > OPTI.ept
            ret_value = ret_value + abs(con_h(i))*Problem.penalty;
        end
    end
end
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
[Fmin, fminindex] = min(MSS.FF(1:VAL.I) + MSS.XX(1:VAL.I));
Xmin = MSS.CC(:, fminindex);

if OPTI.showITS == 1                % Show iteration stats
    VAL.time = toc;
    if MSS.XX(fminindex) > 0
        fprintf(...
...
%--------------------------------------------------------------------------
POH           = size(POHas, 2);
[c_lt, c_rt, f_l, con_l, f_r, con_r, order, s_tp] = deal(cell(1, POH));
li            = (MSS.LL(:, POHas));
oldc          = (MSS.CC(:, POHas));
...
for i = 1:POH
    ...
    con_l{i} = transpose(arrayfun(@(x)...
        CallC(O, point_l(:, x), OPTI), (1:LL(i)).'));
    f_r{i} = transpose(arrayfun(@(x)...
        feval(O.f, point_r(:, x)), (1:LL(i)).'));
    con_r{i} = transpose(arrayfun(@(x)...
        CallC(O, point_r(:, x), OPTI), (1:LL(i)).'));
    [~, order{i}] = sort([min(f_l{i} + con_l{i},...
        f_r{i} + con_r{i})' ls{i}], 1);
    for j = 1:LL(i)
...
for i = 1:POH
    ...
    con_tp = con_r{i}(:, [1;1]*transpose(order{i}(:, 1)));
    con_tp(:, 1:2:end) = con_l{i}(:,order{i}(:, 1));
    MSS.XX(FCX(i) + 1:FCX(i + 1)) = con_tp;                 % g(x)
