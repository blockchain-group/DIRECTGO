function [minima, xatmin, history] = sDirectl(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirectl
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirectl(Problem, bounds, opts)
...
if nargin < 3 && (isempty(opts))
  ...
  getopts(opts, 'maxits', 100,'maxevals', 1000, 'maxdeep', 1000,...
      'testflag', 0, 'globalmin', 0, 'tol', 0.01, 'showits',...
      1, 'ep', 1e-4);
...
OPTI.MAXdeep   = maxdeep;  % maximum number of side divisions
...
MSS.DD(1) = 1/2*norm((1/3*(ones(VAL.n, 1))).^(MSS.LL(:, 1)), inf);
...
for i = 1:size(POH, 2)
    ...
    ss = arrayfun(@(x) 1/2*norm((1/3*(ones(VAL.n, 1))).^(l(:, x)), inf),...
        (1:2*LL));
...
for i =1:hull_length
    tmp_rects = find(sum(hull_lengths, 1) < sum(lengths(:, hull(i))), 1);
