function [ret_minval, final_xatmin, history] = parallel_dglbSolve...
    (Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dglbSolve
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dglbSolve(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
function [MSS, CE, DATA] = subdivision(VAL, Problem,...
    DT, MSS, CE, third, SS)
%--------------------------------------------------------------------------
% Create sets
...
if isempty(find(CE ~= 0, 1)) == 0
    ...
    for i = 1:length_of_MSS
        if CE(i) ~= 0
            ...
            ties = find(MSS(i).F(1:CE(i)) == MV(1, i));
            if ~isempty(ties)
                hul{1, i} = ties;
