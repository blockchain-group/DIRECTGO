function [minima, xatmin, history] = dPlor(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : dPlor
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dPlor(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
function rects = Find_po(MSS, CE, SS, minval, MV)
...
[maybe_po, final_pos, rects] = deal(cell(1, index));
for i = 1:index
...
% PLOR realization --------------------------------------------------------
% largest hyper-rectangle
maxi = find(~cellfun('isempty', final_pos), 1, 'first');
rects{maxi} = final_pos{maxi};

% lowest
mini = find(~cellfun('isempty', final_pos), 1, 'last');
if mini ~= maxi
    rects{mini} = final_pos{mini};
end
return
