function [ret_minval, final_xatmin, history] = parallel_dPlor...
    (Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dPlor
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dPlor(Problem, bounds, opts)
...
spmd (2)                          % Execute code in parallel on labs
%--------------------------------------------------------------------------
    ...
    [MSS, CE, third, VAL, MD] = Alocate(bounds, SS);
...
%--------------------------------------------------------------------------
function [MSS, CE, third, VAL, MD] = Alocate(bounds, SS)
%--------------------------------------------------------------------------
...
% Find minima values
[xmin, fmin, hul] = find_min(CE, MSS, VAL.n);
DATA = {xmin, fmin, hul};
MD = hul(1, :);
%--------------------------------------------------------------------------
labBarrier
...
%--------------------------------------------------------------------------
function [xmin, fmin, hul] = find_min(CE, MSS, n)
%--------------------------------------------------------------------------
if isempty(find(CE ~= 0, 1)) == 0
    ...
    MV = nan(1, length_of_MSS);
    
    for i = 1:length_of_MSS
        if CE(i) ~= 0
            hul{1, i} = find(MSS(i).E(1:CE(i)) == max(MSS(i).E...
                (MSS(i).F(1:CE(i)) == min(MSS(i).F(1:CE(i))))));
            hul{2, i} = nonsize(i);
            hul{3, i} = MSS(i).F(hul{1, i});
            hul{4, i} = labindex;
            hul{5, i} = i;
            MV(i) = MSS(i).F(hul{1, i});
        end
    end

    [fmin, index] = min(MV);
    xmin = MSS(index).C(:, hul{1, index});   
else
    xmin = zeros(n, 1);
    fmin = nan(1, 2);
    hul = cell(5, 1);
...
for i=1:SS.workers
    ...
    MN(i) = D{2}(1);                    % minval
    for j = 1:count(i)
        Md{1, j} = [Md{1, j}, D{3}{1, j}];
        Mz{1, j} = [Mz{1, j}, D{3}{2, j}];
        Mx{1, j} = [Mx{1, j}, D{3}{3, j}];
        My{1, j} = [My{1, j}, D{3}{4, j}];
        Mt{1, j} = [Mt{1, j}, D{3}{5, j}];
    end
end
...
if length(final_pos) >= 3
    final_pos = [final_pos(1), final_pos(end)];
end
return
