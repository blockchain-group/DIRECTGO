function [ret_minval, final_xatmin, history] = parallel_dGb_DIRECT...
    (Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dGb_DIRECT
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dGb_DIRECT(Problem, bounds, opts)
...
%--------------------------------------------------------------------------
    ...
    while VAL.perror > SS.TOL       % Main loop
        [MSS, CE, DT] = subdivision(VAL, Problem, DT, MSS, CE, third, SS);
...
[MSS(1).F(1), fmin, VAL.fMinBeforeImpr] = deal(feval(Problem.f, point));                   
...
%--------------------------------------------------------------------------
function [MSS, CE, DATA] = subdivision(VAL, Problem, DT, MSS, CE, third, SS)
%--------------------------------------------------------------------------
% Create sets
...
[MSS, CE] = STORAGE(Problem, MSS, CE, third, VAL, DT, MAIN);
...
%--------------------------------------------------------------------------
function [MSS,CE] = STORAGE(Problem, MSS, CE, third, VAL, DATA, MAIN)
%--------------------------------------------------------------------------
...
jj    = find(~cellfun(@isempty, Mx(1, :)), 1, 'first');
i_m   = i_min - jj;
 
if (abs(fmin - MVV(i_min)) < 0.01)
    if (abs(fmin - VAL.fMinBeforeImpr) < VAL.Yaro_epsilon*abs(fmin))
...
POH = Find_po([Md; Mz; Mx; My], i_min, i_m, nonsize, VAL.count, MVV,...
    Mx{i_min}(hs(2, i_min)));
...
%--------------------------------------------------------------------------
function S = Find_po(hulls, i_min, i_m, d, index, d_min, b1)
% Find all rects on hub
jj    = find(~cellfun(@isempty, hulls(1, :)), 1, 'first');
JA = i_m + jj;
S_1   = cell(1, index);
S_1(1:JA)   = hulls(1, 1:JA);
Su    = cell(4, JA);
