function [minima, xatmin, history] = Lc_dDISIMPLc(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : Lc_dDISIMPLc
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = Lc_dDISIMPLc(Problem, bounds, opts)
...
[MSS, CE, VAL, V] = Alocate(bounds, SS, Problem);
...
%--------------------------------------------------------------------------
function [MSS, CE, VAL, V] = Alocate(bounds, SS, Problem)
%--------------------------------------------------------------------------
...
for i = 1:VAL.m
    ...
    MSS(1).Simpl(i).dSC = roundn(max(sqrt(sum((MSS(1).Simpl(i).SC -...
                MSS(1).Simpl(i).V(:, 1:VAL.nn)).^2, 1))), -12);
