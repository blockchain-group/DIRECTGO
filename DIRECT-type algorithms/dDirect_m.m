function [minima, xatmin, history] = dDirect_m(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : dDirect_m
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_m(Problem, bounds, opts)
...
third           = zeros(1, VAL.n*SS.MAXdeep);     % delta values
third(1)        = 1/3;                      % first delta
for i = 2:VAL.n*SS.MAXdeep                        % all delta
...
if sum(MSS(idx).L(:,1)) > 1
    for i = 1:index
        if CE(i) ~= 0
            MSS(i).fff = MSS(i).F(1:CE(i));
        end
    end
    fc = [MSS.fff];
    MD = abs(minval - median(fc));
else
    MD = abs(minval);
end
