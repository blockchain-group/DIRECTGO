function [minima, xatmin, history] = parallel_dDirect_Ace(Problem,...
    bounds, opts)
%--------------------------------------------------------------------------
% Function   : parallel_dDirect_Ace
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect_Ace(Problem, bounds, opts)
...
%       A         - aggressive selection of potential optimal rectangles
...
%--------------------------------------------------------------------------
    ...
    [MSS, CE, third, VAL, Q, SS] = Alocate(bounds, SS);
...
%--------------------------------------------------------------------------
function [MSS, CE, third, VAL, Q, SS] = Alocate(bounds, SS)
%--------------------------------------------------------------------------
...
MSV  = ones(4, 1);                              % first fake value
[KK, TT, I, DEL, VAL.fcn] = Decision(1, VAL, MSV, SS);
...
[VR, IR, ER, VF, IF, EF] = deal(nan(SS.workers, SS.aloc));
...
POH_G = 1:zz;
POH_G(isnan(MSV(1, POH_G))) = [];
if VAL.PHASE == 0
    [KK, TT, I, DEL, fcnc] = Decision(POH_G, VAL, MSV, SS);
    ...
    if VAL.epsil ~= 0
        [KK, TT, I, DEL, fcnc] = Decision(POH_G, VAL, MSC, SS);
...
[MSS, CE] = STORAGE(Probl, MSS, CE, third, VAL, DT, MAIN, SS, Q{1});
...
%--------------------------------------------------------------------------
function [MSS, CE] = STORAGE(O, MSS, CE, third, VAL, DATA, MAIN, SS, MV)
%--------------------------------------------------------------------------
...
% Create empty set
TT = nan(3, size(MSS, 2));
Q = {TT, nan(1, 3), zeros(n, 2), nan, TT, nan(2, 1), 0};
            ...
            for i=1:size(MSS, 2)
                if CE(i) ~= 0
                    ...
                    temp = MSS(i).D + MSS(i).cons(1:CE(i)) +...
                                                        MSS(i).F(1:CE(i));
                    Q{1}(2, i) = find(MSS(i).E ==...
                             max(MSS(i).E(temp == min(temp))), 1, 'first');            
                    Q{1}(1, i) = temp(Q{1}(2, i));
                    Q{1}(3, i) = MSS(i).E(Q{1}(2, i));
                end
            end
            for i = 1:size(MSS, 2)
                if CE(i) ~= 0
                    temp = MSS(i).F(1:CE(i)) + (MSS(i).D +...
                            ...
                            >= DT{9});
                    Q{5}(2, i) = find(MSS(i).E ==...
                             max(MSS(i).E(temp == min(temp))), 1, 'first');
                    Q{5}(1, i) = temp(Q{5}(2, i));
                    Q{5}(3, i) = MSS(i).E(Q{5}(2, i));
...
%--------------------------------------------------------------------------
function [KK, TT, I, HH, fnc] = Decision(K, VAL, MSV, SS)
%--------------------------------------------------------------------------

Main = [K; MSV(2, K); VAL.fcn, zeros(1, size(K, 2) - 1); MSV(4, K)];                
