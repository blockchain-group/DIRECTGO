function [minima, xatmin, history] = sDirmin(Problem, bounds, opts)
%--------------------------------------------------------------------------
% Function   : sDirmin
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirmin(Problem, bounds, opts)
...
while VAL.perror > OPTI.TOL                                 % Main loop
    ...
    [VAL, Fmin, Xmin] = Arewedone(OPTI, VAL, MSS, POH, Problem);
end                                                         % End of while
...
[VAL.Xmin, Xmin] = deal(abs(VAL.b - VAL.a).*(MSS.CC(:, 1)) + VAL.a);
[Fmin, MSS.FF(1), VAL.Flocal] = deal(feval(Problem.f, Xmin));        
...
function [VAL, Fmin, Xmin] = Arewedone(OPTI, VAL, MSS, POH, Problem)
%--------------------------------------------------------------------------
options = optimoptions('fmincon', 'StepTolerance', 10^(-10),...
    'Display', 'none');
for i = 1:length(POH)   
    pp = abs(VAL.b - VAL.a).*MSS.CC(:, POH(i)) + VAL.a;
    [Xmin_local, Fmin_local, ~, output] = fmincon(Problem.f, pp, [], [],...
        [], [], VAL.a, VAL.b, [], options);
    VAL.nLs = VAL.nLs + 1;
    VAL.Local = VAL.Local + output.funcCount;
    if (Fmin_local < VAL.Flocal)
        VAL.Flocal = deal(Fmin_local);
        VAL.Xmin = Xmin_local;
    end
end
...
if OPTI.showITS == 1                % Show iteration stats
    ...
    'Iter: %4i  f_min: %15.10f    time(s): %10.05f  local searches: %4i    fn evals: %8i\n',...
        VAL.itctr, Fmin, VAL.time, VAL.nLs, VAL.I + VAL.Local);
end
