...
%
% Purpose 
% This code is prepared for minimization of 80 test problems with
% general constraints.
%
% Source 
...
%
% OUTPUTS
% SolveGeneralProblems_results.mat
%--------------------------------------------------------------------------
...
parent_path = strjoin(parts(1:end), filesep); addpath(parent_path); 
load('Results_on_generally_constrained_problems_(eps = 0.01)');
% -------------------------------------------------------------------------
...
% Results:
% -------------------------------------------------------------------------
SolveGeneralProblems_results = Results3(2:size(Results3, 1), 1:3);
SolveGeneralProblems_results{1, 4} = "Iterations";
SolveGeneralProblems_results{1, 5} = "Evaluations";
SolveGeneralProblems_results{1, 6} = "Seconds";

% Solve problems:
% -------------------------------------------------------------------------
for h = 2:size(SolveGeneralProblems_results, 1)
    % Number of variables
    opts.dimension = SolveGeneralProblems_results{h, 3};   

    % Select problem
    Problem.f = SolveGeneralProblems_results{h, 2};    

    % Solve problem
    [~, ~, history] = dDirect_GLc(Problem, opts);

    % Record algorithm performance results  
    SolveGeneralProblems_results{h, 4} =  history(end, 1);   
    SolveGeneralProblems_results{h, 5} =  history(end, 2);   
    SolveGeneralProblems_results{h, 6} =  history(end, 4);
end

% Store results:
% -------------------------------------------------------------------------
filename = 'SolveGeneralProblems_results.mat'; save(filename);