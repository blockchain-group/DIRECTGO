function [minima, xatmin, history] = dDirect_GLce_min(Problem, opts, bounds)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_GLce_min(Problem, opts, bounds)
...
%--------------------------------------------------------------------------
    ...
    [VAL, SS, Xminval, minval] = Arewedone(minval, VAL, SS,...
        Xminval, Problem, MSS);
...
elseif SS.G_nargout == 3
    xatmin    = CallC(Problem, Xminval, SS);
...
VAL.Xminval = (abs(VAL.b - VAL.a)).*xatmin(:,1) + VAL.a;
...
[VAL, ~, Xminval, minval] = Arewedone(minval, VAL, SS, Xminval,...
    Problem, MSS);
...
%--------------------------------------------------------------------------
function [VAL, SS, Xminval, minval] = Arewedone(minval, VAL, SS,...
    Xminval, Problem, MSS)
%--------------------------------------------------------------------------
...
if minval < VAL.fminloc
    VAL.Xminval = Xminval; 
end
if (VAL.stagnate > 0) && (abs(VAL.min_gen - VAL.fminloc) >...
        0.01*abs(VAL.min_gen)) || VAL.local == 1
    VAL.local = 0;
    options = optimoptions('fmincon', 'Display', 'none', 'ConstraintTolerance', SS.ept);
...
if VAL.fminloc < minval
    xatmin = (VAL.Xminval - VAL.a)./abs(VAL.b - VAL.a);
end
