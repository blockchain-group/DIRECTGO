...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect_GL(Problem, opts, bounds)
...
%--------------------------------------------------------------------------
    ...
    [MV, MD, MSS, CE, third, VAL, SS] = Alocate(SS, VAL);
...
%--------------------------------------------------------------------------
function [MV, MD, MSS, CE, third, VAL, SS] = Alocate(SS, VAL)
%--------------------------------------------------------------------------
...
if size(MSS, 2) ~= 1
    ...
    for i = 1:size(MSS, 2)                                  % DISTANCES
        if CE(i) ~= 0
            ...
            D_eul = sum((XX - MSS(i).C(:, 1:CE(i))).^2, 1).^0.5;
