function [minima, xatmin, history] = sMrDirect075(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sMrDirect
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sMrDirect(Problem, opts, bounds)
...
% identify poh
if level == 2
    POH = Find_po(MSS.FF(1:VAL.I), Fmin, 10^(-5),...
        MSS.DD(1:VAL.I));
elseif level == 1
    I = find(MSS.DD(1:VAL.I) <= quantile(MSS.DD(1:VAL.I), 0.9));
    POH = I(Find_po(MSS.FF(I), Fmin, 10^(-7), MSS.DD(I)));
else
    I = find(MSS.DD(1:VAL.I) <= quantile(MSS.DD(1:VAL.I), 0.1111111111));
    POH = I(Find_po(MSS.FF(I), Fmin, 0, MSS.DD(I)));
end
...
if length(d) - idx > 1
    ...
    for i = 1 : length(S_1)
        j = S_1(i);
        if fc(j) <= slope*szes(j) + const + 1E-8
            Su{i} = j;
        end
    end
    S_2 = cell2mat(Su);
