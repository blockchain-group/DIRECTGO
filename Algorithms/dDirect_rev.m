function [minval, xatmin, history] = dDirect_rev(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : dDirect_rev
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_rev(Problem, opts, bounds)
...
%
% Output parameters:
%       minval  -  best minimum value which was founded
%
...
while VAL.perror > SS.TOL                 % Main loop
    ...
    [VAL, SS, xatmin, minval] = Arewedone(MSS, minval, VAL, SS, xatmin, Problem);
end                                       % End of while
...
MSS(1).E(1) = 1;                                    % Index
Xmin = abs(VAL.b - VAL.a).*MSS(1).C(:, 1) + VAL.a; % initial point
[Fmin, MSS.F(1), VAL.fmin_old, VAL.Flocal, MV(1)] =...
    deal(feval(Problem.f, Xmin));
CE(1) = 1;
...
%--------------------------------------------------------------------------
function [VAL, SS, Xmin, mm] = Arewedone(MSS, mm, VAL, SS, xx, Problem)
%--------------------------------------------------------------------------
Xmin = abs(VAL.b - VAL.a).*xx + VAL.a;

if (abs(mm - VAL.fmin_old) > 0.0001) && mm < VAL.Flocal     
    ...
    if (Fmin_local < mm)
        [mm, VAL.Flocal] = deal(Fmin_local);
        VAL.x_min_general = Xmin_local;
        Xmin = Xmin_local;
    end
end
[mm, indexs] = min([mm, VAL.Flocal]);
...
if SS.TESTflag == 1
    ...
    if SS.globalMIN ~= 0
        VAL.perror = 100*(min(VAL.Flocal, mm) - SS.globalMIN)/abs(SS.globalMIN);
    else
        VAL.perror = 100*min(VAL.Flocal, mm);
    end
...
if SS.G_nargout == 3
    ...
    VAL.history(VAL.itctr, 3) = min(VAL.Flocal, mm);
    VAL.history(VAL.itctr, 4) = VAL.time;
end
