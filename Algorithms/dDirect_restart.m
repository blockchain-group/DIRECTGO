function [minima, xatmin, history] = dDirect_restart(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : dDirect_restart
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dDirect_restart(Problem, opts, bounds)
...
%
% Finkel, D. and C. T. Kelley. “An Adaptive Restart Implementation 
% of DIRECT.” (2004).
%--------------------------------------------------------------------------
...
while VAL.perror > SS.TOL                 % Main loop
    ...
    POH = Find_po(MSS, CE, VAL, minval, MV(1, :));
...
tic                                         % Mesure time
[VAL.ep, VAL.time, VAL.sl] = deal(0);       % initial time
...
MSS(1).E(1) = 1;                                    % Index
[MSS(1).F(1), Fmin, MV(1), VAL.Fmin_old] = deal(feval(Problem.f,...
...
if abs(VAL.Fmin_old - minval) < 10^(-4)
    VAL.sl = VAL.sl + 1;
else
    VAL.sl = 0;
end
...
%--------------------------------------------------------------------------
function final_pos = Find_po(MSS, CE, VAL, minval, MV)
