function [ret_minval, final_xatmin, history] = parallel_dDirect_restart...
    (Problem, opts, bounds)
%--------------------------------------------------------------------------
...
%       restart   - adaptive setting of the epsilen parameter
%
% Input parameters:
...
[MSS(1).F(1), fmin, VAL.Fmin_old] = deal(feval(Problem.f, point));                     

% Divide values for workers
[KK, TT, I, DEL, VAL.fcount] = DIVas(VAL, SS, {1; 1; 1; 1; 1});
% work for master
DATA = {KK, TT, I, size(I,1), DEL, MSS(1).F(1), VAL.ep};
...
% Find minima values
[xmin, fmin, hul] = find_min(CE, MSS, VAL.n, DT{6}, DT{7});
DATA = {xmin, fmin, hul};
MD = hul(1, :);
%--------------------------------------------------------------------------
labBarrier
...
%--------------------------------------------------------------------------
function [xmin, fmin, hul] = find_min(CE, MSS, n, minval, ep)
%--------------------------------------------------------------------------
...
POH = Find_po(VAL.ep, fmin, [Md; Mz; Mx]);
for i = 1:length(POH)
...
DATA = {KK, TT, I, size(I, 1), DEL, fmin, VAL.ep};
...
%--------------------------------------------------------------------------
function final_pos = Find_po(ep, minval, hulls)
