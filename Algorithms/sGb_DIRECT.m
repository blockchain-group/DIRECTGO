function [minima, xatmin, history] = sGb_DIRECT(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sGb_DIRECT
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sGb_DIRECT(Problem, opts, bounds)
...
while VAL.perror > OPTI.TOL                                 % Main loop
    ...
    [POH, VAL] = Find_po(MSS.FF(1:VAL.I), Fmin, OPTI.ep,...
        MSS.DD(1:VAL.I), VAL);
