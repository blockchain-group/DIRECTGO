function [ret_minval, final_xatmin, history] = parallel_dDirect_rev...
    (Problem, opts, bounds)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect_rev(Problem, opts, bounds)
...
%--------------------------------------------------------------------------
    ...
    while VAL.perror > SS.TOL       % Main loop
        [MSS, CE, MV, DT, VAL] = subdivision(VAL, Problem, DT, MSS, CE,...
            third, MV, SS);
        if labindex == 1            % Gather information from workers
            [DT, fmin, VAL, SS, xmin] = Master(VAL, SS, DT, MSS, Problem);
...
MSS(1).Diam = 1/2*norm((1/3*(ones(VAL.n, 1))).^zeros(VAL.n, 1), 2);
for i = 1:SS.MAXdeep - 1
    MSS(i + 1).Diam = 1/2*norm((1/3*...
        (ones(VAL.n, 1))).^(ones(VAL.n, 1)*i), 2);
end
...
MSS(1).E(1) = 1;                                % Index
[point, xmin, VAL.x_min_general] = deal(abs(VAL.b-VAL.a).*MSS(1).C(:, 1) + VAL.a);
[MSS(1).F(1), fmin, VAL.fmin_old, VAL.Flocal] = deal(feval(Problem.f, point));                    
...
if SS.G_nargout == 3
    ...
    VAL.history(VAL.itctr, 5) = VAL.nLs;
end
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
function [MSS, CE, MV, DATA, VAL] = subdivision(VAL, Problem,...
...
[MSS, CE, VAL] = STORAGE(Problem, MSS, CE, third, VAL, DT, MAIN, MV);
...
%--------------------------------------------------------------------------
function [MSS, CE, VAL] = STORAGE(Problem, MSS, CE, third, VAL, DATA, MAIN, MV)
%--------------------------------------------------------------------------
[DD, LIMIT, VAL] = Calulcs(Problem, DATA, MSS, third, VAL, MAIN);
...
% Purpose   : Calculate; new points, function values, lengths and indexes
%--------------------------------------------------------------------------
function [A, LIMIT, VAL] = Calulcs(PR, DATA, MSS, third, VAL, MAIN)
%--------------------------------------------------------------------------
...
for i = 1:LIMIT
    ...
    if length(DIVIS) ~= 1
        [~, indexas] = min(VAL.minas(DIVIS));
        DIVIS = DIVIS(indexas);
    end
    VAL.minas(DIVIS) = VAL.minas(DIVIS) + 1;
...
%--------------------------------------------------------------------------
function [DATA, fmin, VAL, SS, xmin] = Master(VAL, SS, D, MSS, Problem)
%--------------------------------------------------------------------------
