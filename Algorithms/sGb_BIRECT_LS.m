function [minima, xatmin, history] = sGb_BIRECT_LS(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sGb_BIRECT_LS
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sGb_BIRECT_LS(Problem, opts, bounds)
...
while VAL.perror > OPTI.TOL                                 % Main loop
    ...
    POH = Find_po(MSS.FF(1:VAL.I), OPTI.ep, MSS.DD(1:VAL.I), VAL);
    ...
    [VAL, Fmin, Xmin] = Arewedone(OPTI, VAL, MSS, RECT, Problem);
end                                                         % End of while

% Return value
[minima, indexs]  = min([Fmin, VAL.f_min_general]);
...
[MSS.FF(1), Fmin, VAL.fMinBeforeImpr, VAL.fminloc, VAL.f_min_general]...
    = deal(RECT(1).fmin);
...
function [VAL, Fmin, Xmin] = Arewedone(OPTI, VAL, MSS, RECT, Problem)
%--------------------------------------------------------------------------
[Fmin, fminindex] =  min(MSS.FF(1:VAL.I));
Xmin              = RECT(fminindex).pmin;
[issi, indexs]    = min([Fmin, VAL.f_min_general]);
VAL.f_min_general = issi;
XX                = [Xmin, VAL.x_min_general];
VAL.x_min_general = XX(:, indexs);
...
%--------------------------------------------------------------------------
function S = Find_po(fc, epsilon, szes, VAL)
% Find all rects on hub
E          = max(epsilon*abs(VAL.f_min_general), 1E-8);
[~, i_min] = min((fc - VAL.f_min_general + E)./szes);
d          = unique(szes);
