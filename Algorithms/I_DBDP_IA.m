function [minima, xatmin, history] = I_DBDP_IA(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : I_DBDP_IA
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = I_DBDP_IA(Problem, opts, bounds)
%       I_DBDP - Hyper-rectangular partitioning based on 1-Dimensional 
...
while VAL.perror > OPTI.TOL                                 % Main loop
    % Selection of potential optimal hyper-rectangles step
     POH = Find_pohas(MSS.FF(1:VAL.I), MSS.DD(1:VAL.I), 1:VAL.I, VAL);
...
%--------------------------------------------------------------------------
function boxes = Find_pohas(fc, szes, indexs, VAL)
%--------------------------------------------------------------------------
C     = unique(szes);
...
MSS.CC(:, 1) = (RECT(1).p1 + RECT(1).p2)/2;
...
while isempty(POH) == 0
    ...
    MSS.CC(:, VAL.I) = (RECT(VAL.I).p1 + RECT(VAL.I).p2)/2;
    MSS.CC(:, POH(1)) = (RECT(POH(1)).p1 + RECT(POH(1)).p2)/2;
    if OPTI.tighterLB == true
