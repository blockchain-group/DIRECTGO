function [minima, xatmin, history] = dBIRMIN(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : dBIRMIN
...
while VAL.perror > SS.TOL                 % Main loop
    ...
    POH = Find_po(MSS, CE, SS, MV(1, :), VAL);
    ...
    [VAL, SS, xatmin] = Arewedone(minval, VAL, SS, Problem, xatmin);
end                                       % End of while

% Return value
[minima, indexs] = min([minval, VAL.f_min_general]);
XX               = [xatmin, VAL.x_min_general];
xatmin           = XX(:, indexs);
if SS.G_nargout == 3
...
[MSS(1).F(1), Fmin, MV(1), VAL.fMinBeforeImpr, VAL.fminloc,...
    VAL.f_min_general] = deal(MSS(1).RECT(1).fmin);
[Xmin, VAL.x_min_general] = deal(MSS(1).RECT(1).pmin);

MSS(1).L(:, 1) = ones(VAL.n, 1);
MSS(1).Diam = roundn((2/3)*sqrt(sum(MSS.L(:, 1).^2)), -12);
[VAL.fMinNotImpr, VAL.nLocSearch, VAL.nFunc] = deal(0);
...
%--------------------------------------------------------------------------
function [VAL, SS, Xmin] = Arewedone(minval, VAL, SS, Problem, Xmin)
%--------------------------------------------------------------------------
...
Xmin = abs(VAL.b - VAL.a).*Xmin + VAL.a;
VAL.f_min_general = min(minval, VAL.f_min_general);
if (VAL.fMinNotImpr > 0) && (abs(VAL.f_min_general - VAL.fminloc) >...
        0.01*abs(VAL.f_min_general))
 
    options = optimoptions('fmincon', 'Display', 'none');
    [xminloc, VAL.fminloc, ~, output] = fmincon(Problem.f, Xmin, [], [],...
        [], [], VAL.a, VAL.b, [], options);
    VAL.nLocSearch = VAL.nLocSearch + 1;
    
    VAL.nFunc = VAL.nFunc + output.funcCount;
    if (VAL.fminloc < VAL.f_min_general)
        VAL.f_min_general = VAL.fminloc;
        VAL.x_min_general = xminloc;
        Xmin = xminloc;
    end
end
...
if SS.TESTflag == 1
    ...
    if SS.globalMIN ~= 0
        VAL.perror = 100*(VAL.f_min_general - SS.globalMIN)/abs(SS.globalMIN);
    else
        VAL.perror = 100*VAL.f_min_general;
    end
...
if VAL.fcount + VAL.nFunc > SS.MAXevals
...
if SS.G_nargout == 3
    ...
    VAL.history(VAL.itctr, 3) = VAL.f_min_general;
    VAL.history(VAL.itctr, 4) = VAL.time;
end
...
%--------------------------------------------------------------------------
function S = Find_po(MSS, CE, SS, MV, VAL)
...
for i = 1:index
    if CE(i) ~= 0
        ties     = find(MSS(i).F(1:CE(i)) - MV(i) <= 1E-12, 1, 'first');
        if ~isempty(ties)
            hulls{1, i} = ties;
            hulls{2, i} = d(i)*ones(1, size(ties, 2));
            hulls{3, i} = MSS(i).F(ties);
            hulls{4, i} = i*ones(1, size(ties, 2));
            [hs(1, i), hs(2, i)] = min((MSS(i).F(1:CE(i)) -...
                VAL.f_min_general + max(SS.ep*abs(VAL.f_min_general),...
                1E-8))/d(i));
        end
    end
end
...
i_m   = i_min - jj;
if (VAL.fMinNotImpr > 5*VAL.n) && mod(VAL.fMinNotImpr, 5) ~= 0
