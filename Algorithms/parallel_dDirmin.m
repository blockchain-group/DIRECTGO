function [ret_minval, final_xatmin, history] = parallel_dDirmin...
    (Problem, opts, bounds)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
    ...
    while VAL.perror > SS.TOL       % Main loop
        ...
        if labindex == 1            % Gather information from workers
            [DT, fmin, VAL, SS, xmin] = Master(VAL, SS, DT, Problem);
...
TT              = SS{1};
VAL             = VAL{1};
history         = TT.history(1:(VAL.itctr), 1:5); % Return history
%--------------------------------------------------------------------------
return
...
%--------------------------------------------------------------------------
% Create and allocating empty sets
SS.history = zeros(SS.MAXdeep, 5);              % allocating history
...
MSS(1).E(1) = 1;                                % Index
[point, xmin, VAL.Xmin] = deal(abs(VAL.b-VAL.a).*MSS(1).C(:, 1) + VAL.a);
[MSS(1).F(1), fmin, VAL.Flocal] = deal(feval(Problem.f, point));                     
...
if SS.G_nargout == 3
    ...
    SS.history(VAL.itctr, 5) = VAL.nLs;
end
%--------------------------------------------------------------------------
...
if isempty(find(CE ~= 0, 1)) == 0
    ...
    nonsize  = [MSS.Diam];
    [hulls, hul] = deal(cell(6, length_of_MSS));
    
    for i = 1:length_of_MSS
        if CE(i) ~= 0
            [MV(1, i), MV(2, i)] = min(MSS(i).F(1:CE(i)));
            ties = find(abs(MSS(i).F(1:CE(i)) - MV(1, i)) <= 1e-13);
            if ~isempty(ties)
                hulls{1, i} = ties;
...
for i=1:SS.workers
    ...
    for j = 1:count(i)
        ...
        XE{1, j} = [cell2mat(XE(1, j)), cell2mat(D{3}(6, j))];
    end
end
...
VAL = minlocal(XXX, VAL, Problem);

if VAL.Flocal > fmin
    [xmin, VAL.Xmin] = deal(abs(VAL.b-VAL.a).*XR(:, fminindex) + VAL.a);
else
    fmin = VAL.Flocal;
    xmin = VAL.Xmin;
end
...
if SS.G_nargout == 3
    SS.history(VAL.itctr, 1) = VAL.itctr;
    SS.history(VAL.itctr, 2) = VAL.fcount + VAL.Local;
    SS.history(VAL.itctr, 3) = fmin;
    SS.history(VAL.itctr, 4) = VAL.time;
    SS.history(VAL.itctr, 5) = VAL.nLs;
end
...
%--------------------------------------------------------------------------
function VAL = minlocal(MMM, VAL, PP)
%--------------------------------------------------------------------------
options = optimoptions('fmincon', 'Display', 'none');
MMM = cell2mat(MMM);
for i = 1:size(MMM, 2)
    pp = abs(VAL.b - VAL.a).*MMM(:, i) + VAL.a;
    [X_l, F_l, ~, o] = fmincon(PP.f, pp, [], [], [], [], VAL.a, VAL.b, [], options);
    VAL.nLs = VAL.nLs + 1;
    VAL.Local = VAL.Local + o.funcCount;
    if F_l < VAL.Flocal
        VAL.Flocal = F_l;
        VAL.Xmin = X_l;
    end
