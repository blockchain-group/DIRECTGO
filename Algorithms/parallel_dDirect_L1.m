function [ret_minval, final_xatmin, history] = parallel_dDirect_L1...
    (Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : parallel_dDirect
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = parallel_dDirect(Problem, opts, bounds)
...
%
% D. E. Finkel, Global optimization with the Direct algorithm,
% Ph.D. thesis, North Carolina State University (2005).
%
% Constraint handling strategie taken from:
%--------------------------------------------------------------------------
% D. E. Finkel. 2005. Global Optimization with the Direct Algorithm. 
% Ph.D. Dissertation. North Carolina State University.
%
% Parallel scheme taken from:
%--------------------------------------------------------------------------
% Linas Stripinis, Julius Žilinskas, Leocadio G. Casado, and Remigijus 
% Paulavičius. 2021. On MATLAB experience in accelerating DIRECT-GLce
...
%--------------------------------------------------------------------------
    ...
    [MSS, CE, third, VAL, MD, SS] = Alocate(SS, VAL);
    ...
    while VAL.perror > SS.TOL       % Main loop
        [MSS, CE, DT, MD] = subdivision(VAL, Problem,...
            DT, MSS, CE, third, SS, MD);
...
%--------------------------------------------------------------------------
function [MSS, CE, third, VAL, MD, SS] = Alocate(SS, VAL)
%--------------------------------------------------------------------------
...
MSS(1).F(1) = deal(feval(Problem.f, point)); 
MSS(1).X(1) = CallC(Problem, point, SS);

fmin(1) = MSS(1).X(1) + MSS(1).F(1);
fmin(2) = MSS(1).X(1);
...
%--------------------------------------------------------------------------
function [MSS, CE, DATA, MD] = subdivision(VAL, Problem,...
    DT, MSS, CE, third, SS, MD)
%--------------------------------------------------------------------------
% Create sets
...
if ~isempty(excess)                            % Is any excess POH on Worker
    if ~isempty(DT{2}{labindex})
        ...
        % Receive information
        if labindex == lack(1)
            [MO.F, MO.X, MO.O, MO.E, MO.fef, MO.L, MO.C] = deal([]);
        ...
        MAIN.XX = MO.X(1:xc); MO.X = MO.X(xc + 1:end);
...
[MSS, CE] = STORAGE(Problem, MSS, CE, third, VAL, DT, MAIN, MD, SS);
...
%--------------------------------------------------------------------------
function [MSS,CE] = STORAGE(Problem, MSS, CE, third, VAL, DATA, MAIN, MV, SS)
%--------------------------------------------------------------------------
[DD, LIMIT] = Calulcs(Problem, DATA, MSS, third, VAL, MAIN, SS);
...
for i = 1:LIMIT
    for h = 1:DD{i}.S
        ...
        if CE(II) == 0
            ...
            [MSS(II).F, MSS(II).X, MSS(II).E] = deal(zeros(1, 100));
        end
        ...
        MSS(II).X(IL) = DD{i}.Lx(DD{i}.lsx(h));         % Left f(x)
        MSS(II).X(IR) = DD{i}.Rx(DD{i}.lsx(h));         % Right f(x)
    ...
    MSS(II).X(CE(II)) = DD{i}.Ox;                   % Center f(x)
    ...
    for i = 1:size(remm, 2)
        ...
        if (CE(I) - size(remov, 2)) == 0
            if find(CE ~= 0, 1, 'first') == I
                [MSS(I).E, MSS(I).L, MSS(I).F, MSS(I).X, MSS(I).C] = deal([]);
            end
        else
...
% Purpose   : Calculate; new points, function values, lengths and indexes
%--------------------------------------------------------------------------
function [A, LIMIT] = Calulcs(PR, DATA, MSS, third, VAL, MAIN, SS)
%--------------------------------------------------------------------------
...
for i = 1:LIMIT_w
    ...
    A{i}.Ox = MSS(A{i}.mdx).X(DATA{1}{labindex}(2, i));
...
for i = 1:LIMIT
    ...
    A{i}.S = length(DIVIS);
    [A{i}.L, A{i}.R, A{i}.Lx, A{i}.Rx, A{i}.eL, A{i}.eR] = deal(zeros(1, A{i}.S));
    ...
    for g = 1:A{i}.S
        ...
        A{i}.L(g) = feval(PR.f, Point);
        A{i}.Lx(g) = CallC(PR, Point, SS);
        A{i}.cR(DIVIS(g), g) = A{i}.cR(DIVIS(g), g) + DELTA;
        Point = abs(VAL.b - VAL.a).*A{i}.cR(:, g) + VAL.a;
        A{i}.R(g) = feval(PR.f, Point);
        A{i}.Rx(g) = CallC(PR, Point, SS);
    end
    [~, A{i}.lsx] = sort([min(A{i}.L + A{i}.Lx, A{i}.R + A{i}.Rx)' DIVIS], 1);
    for g = 1:A{i}.S
...
if isempty(find(CE ~= 0, 1)) == 0
    ...
    nonsize  = [MSS.Diam];
    [hulls, hul] = deal(cell(5, length_of_MSS));
    
    for i = 1:length_of_MSS
        if CE(i) ~= 0
            [MV(1, i), MV(2, i)] = min(MSS(i).F(1:CE(i)) + MSS(i).X(1:CE(i)));
            ties = find(MSS(i).F(1:CE(i)) + MSS(i).X(1:CE(i)) ==...
                min(MSS(i).F(1:CE(i)) + MSS(i).X(1:CE(i))));
            if ~isempty(ties)
                hulls{1, i} = ties;
                hulls{2, i} = nonsize(i)*ones(1, size(ties, 2));
                hulls{3, i} = MSS(i).F(ties) + MSS(i).X(ties);
                hulls{4, i} = labindex*ones(1, size(ties, 2));
                hulls{5, i} = i*ones(1, size(ties, 2));
            end
        end
        
    end

    [fmin(1), index] = min(MV(1, :));
    xmin = MSS(index).C(:,MV(2, index));
    fmin(2) = MSS(index).X(MV(2, index));
    minval = min([fmin(1), minval]);
    POH = Find_po(SS, minval, hulls);
    for i = 1:length(POH)
        hul{1, POH(i)} = hulls{1, POH(i)};
        hul{2, POH(i)} = hulls{2, POH(i)};
        hul{3, POH(i)} = hulls{3, POH(i)};
        hul{4, POH(i)} = hulls{4, POH(i)};
        hul{5, POH(i)} = hulls{5, POH(i)};
    end    
else
    xmin = zeros(n, 1);
    fmin = nan(1, 2);
...
[count, LB, MN, CC]  = deal(ones(1, SS.workers));
...
if SS.showITS == 1
    ...
    if constraint ~= 0
        fprintf(...
            ...
            VAL.itctr, fmin(1), VAL.time, VAL.fcount);
    else
        fprintf(...
            'Iter: %4i   f_min: %15.10f    time(s): %10.05f    fn evals: %8i\n',...
            VAL.itctr, fmin(1), VAL.time, VAL.fcount);
...
%--------------------------------------------------------------------------
function final_pos = Find_po(SS, minval, hulls)
