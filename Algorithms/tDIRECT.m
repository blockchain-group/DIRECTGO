function [minima, xatmin, history] = tDIRECT(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : stDIRECT
...
% References:
%--------------------------------------------------------------------------
% Kanayama, K., Seko, A., Toyoura, K. (2023). Structure search method 
% for atomic clusters based on the dividing rectangles algorithm. 
% Phys. Rev. E, 108, 035303. https://doi.org/10.1103/PhysRevE.108.035303.
%--------------------------------------------------------------------------
...
% Initialization step
[OPTI, VAL, MSS] = Initialization(VAL, OPTI, Problem, MSS);

while VAL.perror > OPTI.TOL                                 % Main loop
    % Selection of potential optimal hyper-rectangles step
    POH = Find_po(MSS.FF(1:VAL.I), VAL.Fmin, OPTI.ep, MSS.DD(1:VAL.I));
...
getopts(opts, 'maxits', 1000,'maxevals', 100000, 'maxdeep', 1000,...
    ...
    1e-4, 'globalmin', 0, 'globalxmin', 0, 'ftarget', -inf);
...
VAL.opts = optimoptions('fmincon', 'MaxFunEvals', 1e2*VAL.n,...
            'MaxIter', 1e2*VAL.n, 'Display', 'off', 'Algorithm', 'sqp');
% alociate MAIN sets
z   = round(OPTI.MAXevals/1000);
...
%--------------------------------------------------------------------------
function [OPTI, VAL, MSS] = Initialization(VAL, OPTI, Problem, MSS)
%--------------------------------------------------------------------------
VAL.I  = 1;                                       % evaluation counter
MSS.DD(1) = 1;                                    % initial diameter
MSS.CC(:, 1) = ones(VAL.n, 1)/2;                  % initial midpoint
VAL.E = @(x) abs( VAL.b - VAL.a ) .* x + VAL.a;

VAL.Xmin = VAL.E(MSS.CC(:, 1));
[VAL.Fmin, MSS.FF(1)] = deal(feval(Problem.f, VAL.Xmin));        
[Xmin_local, Fmin_local, ~, output] = fmincon(Problem.f, VAL.Xmin, [],...
            [], [], [], VAL.a, VAL.b, [], VAL.opts);

VAL.nLs = VAL.nLs + 1;
VAL.Local = VAL.Local + output.funcCount;
if (Fmin_local < VAL.Fmin)
    VAL.Fmin = Fmin_local;
    VAL.Xmin = Xmin_local;
end
...
function VAL = Arewedone(OPTI, VAL, MSS)
%--------------------------------------------------------------------------
VAL.time = toc;
...
if OPTI.showITS == 1                % Show iteration stats
    ...
    'Iter: %4i  f_min: %15.10f    time(s): %10.05f  local searches: %4i    fn evals: %8i\n',...
        VAL.itctr, VAL.Fmin, VAL.time, VAL.nLs, VAL.I + VAL.Local);
end
...
for i = 1:size(POH, 2)
    ...
    ff = zeros(1, 2*LL);
    for j = 1:2*LL  
        [Xmin_local, ff(j), ~, output] = fmincon(O.f, point(:, j), [],...
            [], [], [], VAL.a, VAL.b, [], VAL.opts);
        VAL.nLs = VAL.nLs + 1;
        VAL.Local = VAL.Local + output.funcCount;
        if (ff(j) < VAL.Fmin)
    ...
    [~, order] = sort([min(ff(1:2:end), ff(2:2:end))' ls], 1);
