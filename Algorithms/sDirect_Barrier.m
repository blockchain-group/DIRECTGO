function [minima, xatmin, history] = sDirect_Barrier(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sDirect_Barrier
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_Barrier(Problem, opts, bounds)
...
%--------------------------------------------------------------------------
  ...
  Xmin           = abs(VAL.b - VAL.a).*(MSS.CC(:, 1)) + VAL.a;      
  if isfield(Problem, 'constraint')
      MSS.XX(1) = CallC(Problem, Xmin, OPTI);
      if MSS.XX(1) == 0
          MSS(1).FF(1) = deal(feval(Problem.f, Xmin));
      else
          MSS(1).FF(1) = deal(10^9);
      end
  else
      ff = feval(Problem.f, Xmin);
