function [minima, xatmin, history] = sDirect_a(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sDirect_a
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_a(Problem, opts, bounds)
...
if sum(lengths,1) > 1
    if minval ~= 0
        po = find((minval - fc(hull(maybe_po)))./abs(minval - mean(fc))...
