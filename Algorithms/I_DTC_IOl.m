function [minima, xatmin, history] = I_DTC_IOl(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : I_DTC_IOl
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = I_DTC_IOl(Problem, opts, bounds)
...
% References:
%--------------------------------------------------------------------------
% Jones, D.R., Martins, J.R.R.A. The DIRECT algorithm: 25 years Later. 
% J Glob Optim 79, 521–566 (2021). https://doi.org/10.1007/s10898-020-00952-6
%--------------------------------------------------------------------------
...
while VAL.perror > OPTI.TOL                                 % Main loop
    ...
    POH = Find_po(MSS.FF(1:VAL.I), Fmin, OPTI.ep, MSS.DD(1:VAL.I)); %original
...
for i = 1:size(POH, 2)
    ...
    MSS.DD(POH(i)) = norm((1/3*(ones(VAL.n, 1))).^(MSS.LL(:, POH(i))), inf);
