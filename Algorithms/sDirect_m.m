function [minima, xatmin, history] = sDirect_m(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : sDirect_m
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = sDirect_m(Problem, opts, bounds)
...
third       = zeros(1, VAL.n*OPTI.MAXdeep);   % delta values
third(1)    = 1/3;                      % first delta
for i = 2:VAL.n*OPTI.MAXdeep                  % all delta
...
if sum(lengths,1) > 1
    if minval ~= 0
        po = find((minval - fc(hull(maybe_po)))./abs(minval - median(fc))...
            + szes(hull(maybe_po)).*ubound(maybe_po)./abs(minval -...
            median(fc)) >= ep);
    else
