function [minima, xatmin, history] = dglbSolve(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : dglbSolve
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = dglbSolve(Problem, opts, bounds)
    ...
    if ~isempty(PH{i})
        for j = 1:size(PH{i}, 2)
            ...
            for h = 1:TT
                ...
                if CE(II) == 0
                    MSS(II).Diam = round(sqrt(sum(DD.lL(:, h).^2)), 15);
